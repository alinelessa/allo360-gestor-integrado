...
const Clientes = () => {
  ...
  const [form, setForm] = useState({ nome: "", cpf_cnpj: "", telefone: "", email: "", endereco: "", limite_credito: "", status: "ativo" });

  const empresaId = profile?.empresa_id;

  const fetchData = async () => {
    if (!empresaId) return;
    const { data } = await supabase.from("clientes").select("*").eq("empresa_id", empresaId).is("deleted_at", null).order("nome");
  ...
  const resetForm = () => {
    setOpen(false);
    setEditing(null);
    setForm({ nome: "", cpf_cnpj: "", telefone: "", email: "", endereco: "", limite_credito: "", status: "ativo" });
  };

  const openEdit = (c: Cliente) => {
    setEditing(c);
    setForm({ nome: c.nome, cpf_cnpj: c.cpf_cnpj || "", telefone: c.telefone || "", email: c.email || "", endereco: c.endereco || "", limite_credito: String(c.limite_credito), status: c.status });
    setOpen(true);
  };
            ...
            <TableBody>
              {filtered.map((c) => (
                <TableRow key={c.id}>
                  ...
                  <TableCell className="text-right">R$ {Number(c.limite_credito).toFixed(2)}</TableCell>
